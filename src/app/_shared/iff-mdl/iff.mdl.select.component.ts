...
import { MdlPopoverComponent } from '@angular-mdl/popover/index';
import { MdlOptionComponent } from '@angular-mdl/select/option';
import { Key, isKey, isCharacterKey, keyboardEventKey } from '@angular-mdl/select/keyboard';
...
export const MDL_SELECT_VALUE_ACCESSOR: any = {
    ...
    useExisting: forwardRef(() => IffMdlSelectComponent),
    multi: true
};
...
export class IffMdlSelectComponent extends SearchableComponent implements ControlValueAccessor, AfterContentInit, AfterViewInit, OnChanges {
    @Input() topPosition: boolean = true;
    @Input() ngModel: any;
    ...
    @Input() AllowHideOnClick: boolean = true;
    @Input() IsNewBrand = false;
    @Input() IsNewSubBrand = false;
    @Input() ShowNewBrandPopup = false;
    @Output() selectNewBrandOption: EventEmitter<any> = new EventEmitter(true);
    @Output() selectNewSubBrandOption: EventEmitter<any> = new EventEmitter(true);
    @Output() closeNewBrandSelection: EventEmitter<number> = new EventEmitter(true);
    @Output() newBrandEntered: EventEmitter<Array<string>> = new EventEmitter(true);
    @Output() newSubBrandEntered: EventEmitter<string> = new EventEmitter(true);
    ...
    @Output() onFocusMdlSelect: EventEmitter<any> = new EventEmitter(true);
    @ViewChild('selectInput') selectInput: ElementRef;
    @ViewChild('newBrandForm') newBrandForm: ElementRef;
    ...
    private newBrand: string = null;
    private newSubBrand: string = null;
    directionUp = false;
    @Input() alwaysBottom = false;
    ...
    private transform = this.upperCasePipe.transform;
    ...
    public ngOnChanges(changes: SimpleChanges) {
        if (changes.AllowHideOnClick) {
            if (!changes.AllowHideOnClick.currentValue) {
                // No need to run change detection here
            }
        }
    }

    public selectedNewBrand() {
        this.AllowHideOnClick = false;
        this.selectNewBrandOption.emit(0);
    }

    public selectedNewSubBrand() {
        this.AllowHideOnClick = false;
        this.selectNewSubBrandOption.emit('0001');
    }

    public closeNewBrandPopover(type = 1) {
        this.closeNewBrandSelection.emit(type);
    }

    public changeToUpper(textString: string, model: string) {
        switch (model) {
            case 'newBrand':
                this.newBrand = this.transform(textString.trim());
                break;
            case 'newSubBrand':
                this.newSubBrand = this.transform(textString.trim());
                break;
        }
    }

    public createNewBrandAndSubbrand() {
        if (this.IsNewBrand && !this.IsNewSubBrand) {
            if (this.newBrand !== undefined && this.newSubBrand !== undefined) {
                this.AllowHideOnClick = true;
                this.closeNewBrandPopover(0);
                if (this.newSubBrand === undefined) {
                    this.newSubBrand = '';
                }
                this.newBrandEntered.emit([this.newBrand, this.newSubBrand]);
            }
        } else {
            if (this.newSubBrand !== undefined) {
                this.AllowHideOnClick = true;
                this.closeNewBrandPopover(0);
                this.newSubBrandEntered.emit(this.newSubBrand);
            }
        }
    }
    ...
    public isRequestDisabled(): boolean {
        if (this.IsNewBrand && !this.IsNewSubBrand) {
            return !(this.newBrand && this.newSubBrand);
        } else if (this.IsNewSubBrand && !this.IsNewBrand) {
